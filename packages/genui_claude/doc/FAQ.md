...
Common questions and troubleshooting for genui_claude.
...
### What is genui_claude?

genui_claude is a Flutter package that implements the GenUI SDK's `ContentGenerator` interface for Claude AI. It enables Claude to generate dynamic, interactive UIs in Flutter applications using the A2UI (Agent to UI) protocol.
...
generator.errorStream.listen((error) {
  final exception = error['exception'] as ClaudeException?;
