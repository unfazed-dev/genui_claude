...
Practical code examples for using the `genui_claude` package.
...
```bash
flutter run --dart-define=CLAUDE_API_KEY=sk-ant-api03-xxxxx
```
...
class ConversationManager {
  final ClaudeContentGenerator generator;
...
class MockContentGenerator extends Mock implements ClaudeContentGenerator {}
  ...
  group('Integration tests', () {
    ...
    setUp(() {
      generator = ClaudeContentGenerator(
        apiKey: apiKey,
        config: const ClaudeConfig(maxTokens: 256), // Smaller for tests
      );
    });
