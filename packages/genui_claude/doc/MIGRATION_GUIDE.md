...
This guide helps you migrate between versions of the `genui_claude` package.
...
- **a2ui_claude**: A2UI protocol parsing and message types
...
2. **Exception Hierarchy Refinements**
   ...
   - Migration: Use pattern matching with `ClaudeException` base type
...
ContentGenerator createGenerator() {
  if (FeatureFlags.useNewGenerator) {
    return ClaudeContentGenerator.proxy(/* new config */);
  } else {
...
dependencies:
  genui_claude: 0.1.0  # Exact version, no caret
```
