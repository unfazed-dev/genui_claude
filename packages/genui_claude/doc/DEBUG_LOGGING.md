...
Configure logging for debugging and production monitoring in genui_claude.
...
genui_claude uses Dart's `logging` package for structured logging. Logs include:
...
genui_claude uses these log levels:
...
genui_claude uses these logger names:
- `DirectModeHandler`
- `ProxyModeHandler`
- `CircuitBreaker`
...
void setupFileLogging() async {
  final logFile = File('${Directory.systemTemp.path}/genui_claude.log');
