...
This document provides a comprehensive overview of test coverage for the `genui_anthropic` package.
...
|-----------|------|-------|----------|
| AnthropicConfig | `config/config_validation_test.dart` | 26 | Defaults, custom values, presets, copyWith, immutability |
...
| Component | File | Tests | Coverage |
|-----------|------|-------|----------|
| NetworkException | `exceptions/anthropic_exceptions_test.dart` | 7 | Creation, isRetryable, typeName, toString |
| TimeoutException | `exceptions/anthropic_exceptions_test.dart` | 4 | Creation, isRetryable, typeName |
| AuthenticationException | `exceptions/anthropic_exceptions_test.dart` | 6 | 401/403 status codes, isRetryable |
| RateLimitException | `exceptions/anthropic_exceptions_test.dart` | 6 | Retry-After header, isRetryable |
| ValidationException | `exceptions/anthropic_exceptions_test.dart` | 5 | 400/422 status codes, isRetryable |
| ServerException | `exceptions/anthropic_exceptions_test.dart` | 5 | 5xx status codes, isRetryable |
| StreamException | `exceptions/anthropic_exceptions_test.dart` | 4 | Creation, isRetryable |
| CircuitBreakerOpenException | `exceptions/anthropic_exceptions_test.dart` | 5 | Recovery time, isRetryable |
| ExceptionFactory | `exceptions/anthropic_exceptions_test.dart` | 18 | HTTP status mapping, Retry-After parsing |
...
| Component | File | Tests | Coverage |
|-----------|------|-------|----------|
| AnthropicContentGenerator | `widget/anthropic_content_generator_test.dart` | 30+ | Factory methods, streaming, error handling |
...
flutter test test/exceptions/anthropic_exceptions_test.dart

# Run with verbose output
flutter test --reporter expanded

# Run integration tests (requires API key)
TEST_ANTHROPIC_API_KEY=your-key flutter test test/integration/real_api_test.dart
```
...
├── exceptions/
│   └── anthropic_exceptions_test.dart
├── handler/
...
These tests are skipped in CI unless `TEST_ANTHROPIC_API_KEY` is provided:

- `real_api_test.dart` - Tests against actual Anthropic API
