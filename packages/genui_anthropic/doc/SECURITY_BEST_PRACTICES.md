...
This guide outlines security best practices for using the `genui_anthropic` package in production applications.
...
The Anthropic API key is a sensitive credential that provides full access to your account. **Never include it in client-side code.**
...
1. Store the Anthropic API key securely (environment variables, secret manager)
