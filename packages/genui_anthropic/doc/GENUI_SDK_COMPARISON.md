# GenUI SDK vs genui_anthropic: Deep-Dive Comparison

This document provides a comprehensive comparison between the GenUI SDK (`genui: ^0.5.1`) and the `genui_anthropic` package, detailing how each GenUI SDK feature is implemented.
...
| `genui_anthropic` | ContentGenerator implementation for Claude AI | 0.1.0 |

The `genui_anthropic` package implements the GenUI SDK's `ContentGenerator` interface, enabling Claude-powered generative UI in Flutter applications.

## Quick Reference

### Compatibility Matrix

| GenUI SDK Feature | genui_anthropic Support | Notes |
|-------------------|------------------------|-------|
...
The `AnthropicContentGenerator` class implements the `ContentGenerator` interface from GenUI SDK.

#### Properties

| GenUI Property | genui_anthropic Implementation |
|----------------|--------------------------------|
...
| GenUI Method | genui_anthropic Implementation |
|--------------|--------------------------------|
...
```dart
// genui_anthropic/lib/src/content_generator/anthropic_content_generator.dart
...
The `A2uiMessageAdapter` class converts anthropic_a2ui messages to GenUI `A2uiMessage` types.
...
| Behavior | GenUI SDK Expectation | genui_anthropic Implementation |
|----------|----------------------|--------------------------------|
...
| Scenario | GenUI SDK Expectation | genui_anthropic Implementation |
|----------|----------------------|--------------------------------|
...
| State | GenUI SDK Expectation | genui_anthropic Implementation |
|-------|----------------------|--------------------------------|
...
final conversation = GenUiConversation(
  contentGenerator: AnthropicContentGenerator(apiKey: 'key'),
  catalog: MyCatalog(),
);
...
These tools are defined in the [anthropic_a2ui](../../../anthropic_a2ui) package and automatically registered when using `AnthropicContentGenerator`.
