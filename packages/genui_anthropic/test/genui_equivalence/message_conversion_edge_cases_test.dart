...
/// ensuring genui_anthropic correctly handles all GenUI SDK message types.
@TestOn('vm')
library;
  ...
  group('GenUI Message Conversion Edge Cases', () {
    group('UserMessage with Image Parts', () {
      test('converts UserMessage with base64 image', () {
        ...
        expect(content[0]['text'], 'Here is an image:');

        // Image block
        expect(content[1]['type'], 'image');
        expect(content[1]['source']['type'], 'base64');
        expect(content[1]['source']['media_type'], 'image/png');
        expect(content[1]['source']['data'], base64Data);
      });
      ...
      test('converts UserMessage with URL image', () {
        ...
        // Image with URL
        expect(content[1]['type'], 'image');
        expect(content[1]['source']['type'], 'url');
        expect(content[1]['source']['url'], 'https://example.com/image.jpg');
      });
    ...
    group('Tool Response Formatting', () {
      test('converts single ToolResultPart', () {
        ...
        expect(content[0]['tool_use_id'], 'call-123');
        expect(content[0]['content'], 'Tool output');
      });
      ...
      test('converts multiple ToolResultParts in one message', () {
        ...
        expect(content[0]['tool_use_id'], 'call-1');
        expect(content[1]['tool_use_id'], 'call-2');
        expect(content[2]['tool_use_id'], 'call-3');
      });
      ...
      test('converts ToolResultPart with complex JSON result', () {
        ...
        final content = result[0]['content'] as List<dynamic>;
        expect(content[0]['content'], complexResult);
      });
      ...
      test('converts ToolResultPart with very long result', () {
        ...
        final content = result[0]['content'] as List<dynamic>;
        expect(content[0]['content'], longResult);
        expect((content[0]['content'] as String).length, 10000);
      });
    });
    ...
    group('AiTextMessage with ToolCallPart', () {
      test('converts AiTextMessage with text and tool call', () {
        ...
        // Text block
        expect(content[0]['type'], 'text');
        expect(content[0]['text'], 'Let me search for that.');

        // Tool use block
        expect(content[1]['type'], 'tool_use');
        expect(content[1]['id'], 'tool-1');
        expect(content[1]['name'], 'search');
        expect(content[1]['input'], {'query': 'flutter'});
      });
      ...
      test('converts AiTextMessage with multiple tool calls', () {
        ...
        final content = result[0]['content'] as List<dynamic>;
        expect(content, hasLength(2));
        expect(content[0]['name'], 'begin_rendering');
        expect(content[1]['name'], 'surface_update');
      });
