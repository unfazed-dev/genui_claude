...
| Package Path | packages/a2ui_claude |

---

## 1. Executive Summary

The a2ui_claude package provides a pure Dart implementation for converting between Claude API responses and the A2UI (Agent-to-UI) protocol messages. This package serves as the foundational layer for integrating Claude's language model capabilities with generative UI frameworks.
...
There is no existing solution for using Claude models with the GenUI framework. This creates a significant limitation for developers who prefer Claude's capabilities or require vendor diversity in their AI implementations.
...
│  (genui_claude)│  (API backend)  │  (Supabase/Cloudflare)     │
└────────┬────────┴────────┬────────┴──────────┬──────────────────┘
         │                 │                   │
         └─────────────────┴───────────────────┘
                           │
...
├── packages/
│   ├── a2ui_claude/                   # This package
│   │   └── ...
│   └── genui_claude/                  # Flutter package (depends on this)
│       └── ...
├── example/
...
```
packages/a2ui_claude/
├── lib/
│   ├── a2ui_claude.dart              # Public API exports
│   └── src/
│       ├── converter/
...
- genui_claude - Flutter ContentGenerator using this package (same monorepo)
