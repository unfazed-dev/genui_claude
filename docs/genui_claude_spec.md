...
| GenUI Version | ^0.5.1 |
| License | MIT |
| Repository | github.com/unfazed-dev/genui_claude (monorepo) |
| Package Path | packages/genui_claude |

---

## 1. Executive Summary

The genui_claude package provides a Flutter-native ContentGenerator implementation that enables Claude AI models to power the Flutter GenUI SDK. This package serves as the bridge between Claude's sophisticated reasoning capabilities and GenUI's dynamic widget rendering system.

By leveraging the a2ui_claude Dart package for protocol conversion, genui_claude provides a thin, focused adapter layer that integrates seamlessly with the existing GenUI ecosystem while maintaining compatibility with Flutter's reactive programming patterns.
...
This package provides ClaudeContentGenerator, a production-ready ContentGenerator implementation that enables Claude-powered generative UI in Flutter applications. It supports both direct API access for development and backend proxy patterns for production deployments.
...
- **Thin Adapter Layer:** Minimal code wrapping a2ui_claude functionality
...
### 3.3 Relationship to a2ui_claude

This package maintains a clear separation of concerns with a2ui_claude:
...
class ClaudeContentGenerator implements ContentGenerator {
  ...
  factory ClaudeContentGenerator({
  ...
  factory ClaudeContentGenerator.proxy({
...
Bridges between a2ui_claude message types and GenUI's A2uiMessage types.

```dart
class A2uiMessageAdapter {
  /// Convert a2ui_claude message to GenUI A2uiMessage
...
class ResilientChatView extends StatelessWidget {
  ...
  final ClaudeContentGenerator contentGenerator;
...
void main() {
  testWidgets('ChatScreen renders generated UI', (tester) async {
    final mockGenerator = MockClaudeContentGenerator();
...
void main() {
  ...
  testWidgets('Full Claude → GenUI flow', (tester) async {
    ...
    final generator = ClaudeContentGenerator(
      apiKey: Platform.environment['TEST_CLAUDE_API_KEY']!,
      model: 'claude-sonnet-4-20250514',
...
dependencies:
  ...
  a2ui_claude: ^1.0.0           # Protocol conversion (Dart package)
...
```
genui_claude/                          # Monorepo root
├── packages/
│   ├── a2ui_claude/                   # Dart package (protocol conversion)
│   │   └── ...
│   └── genui_claude/                  # This package
│       └── ...
├── example/
...
```
packages/genui_claude/
├── lib/
│   ├── genui_claude.dart           # Public API exports
│   └── src/
│       ├── content_generator/
...
- a2ui_claude - Protocol conversion layer (same monorepo)
...
This package is developed as part of the genui_claude monorepo. For local development:

```bash
# Clone the monorepo
git clone https://github.com/unfazed-dev/genui_claude.git
cd genui_claude
