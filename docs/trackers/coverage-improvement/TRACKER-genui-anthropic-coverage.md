# TRACKER: genui_anthropic Package Coverage

## Status: COMPLETED

## Overview
Improve genui_anthropic package test coverage to 100%.
...
| anthropic_content_generator.dart | 117-129, 150-160, 161-168 | Race condition + event handling |
| anthropic_exceptions.dart | 182-191, 223 | Unused StreamException + private constructor |
...
1. **SDK Integration Code** - `direct_mode_handler.dart` makes live API calls to Anthropic's servers. Testing would require mocking the entire SDK or using real API keys (integration tests).
...
## Notes
- Test file location: `packages/genui_anthropic/test/`
- Using flutter_test for testing
- Coverage exclusions follow Dart coverage:ignore-line/start/end syntax
- Integration tests with `TEST_ANTHROPIC_API_KEY` cover SDK integration code
