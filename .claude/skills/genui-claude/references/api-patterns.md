...
Detailed patterns for using the genui_claude API, including handler customization, stream processing, and message conversion.

## ClaudeContentGenerator Patterns
...
final generator = ClaudeContentGenerator(
  ...
  config: ClaudeConfig(maxTokens: 4096),
);
...
The `ClaudeStreamHandler` (from a2ui_claude) processes raw events:
...
Sealed `ClaudeException` enables exhaustive error handling with pattern matching:
...
_generator.errorStream.listen((error) {
  ...
  if (exception is ClaudeException) {
    // Check retryability
...
test('sends message and receives UI', () async {
  ...
  final generator = ClaudeContentGenerator.withHandler(handler: handler);
